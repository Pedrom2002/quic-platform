...
export async function addExistingClientAction(
  ...
  const { error } = await supabase
    .from('event_clients')
    .insert({ event_id: eventId, client_id: clientId, role, notification_prefs: { channels: ['email', 'sms', 'portal'], language: 'pt' } })
...
export async function createAndAddClientAction(
  ...
  const { error: ece } = await supabase
    .from('event_clients')
    .insert({ event_id: eventId, client_id: client.id, role, notification_prefs: { channels: ['email', 'sms', 'portal'], language: 'pt' } })

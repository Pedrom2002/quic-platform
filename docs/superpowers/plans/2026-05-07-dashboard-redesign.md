# Dashboard Editorial Premium Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `app/dashboard/page.tsx` JSX with an editorial premium layout: dark gradient header with greeting + KPIs, light `#f5f5f5` body with events and notifications cards.

**Architecture:** Single file change — `app/dashboard/page.tsx`. Data fetching (all `Promise.all` queries) is unchanged. Only the returned JSX is replaced.

**Tech Stack:** Next.js App Router (Server Component), Tailwind CSS, `next/image`, `lucide-react`, `date-fns`

---

### Task 1: Replace dashboard JSX with editorial premium layout

**Files:**
- Modify: `app/dashboard/page.tsx`

This is a pure JSX/visual change. The data fetching block (lines 11-46) stays identical. Only the `return (...)` block changes.

- [ ] **Step 1: Read the current file to understand structure**

Read `app/dashboard/page.tsx` lines 1-60 to confirm imports and data shape before editing.

- [ ] **Step 2: Replace the return block**

Keep everything above `return (` unchanged. Replace `return (...)` with:
...
- [ ] **Step 3: Add Image import if missing**

Check line 1-10 of the file. If `Image` from `'next/image'` is not already imported, add it:
...
npx tsc --noEmit
```
Expected: no errors. If errors about missing imports, add them.

- [ ] **Step 5: Commit**

```bash
git add app/dashboard/page.tsx
git commit -m "feat: dashboard editorial premium redesign"
git push
```
